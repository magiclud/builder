...
*
*/
public class ReportAssembler implements JSONReportBudowniczy {
	...
	private JSONReportBody jsonReport;
	private XMLReportBody xmlReport;

	Report report = new Report();

	public ReportAssembler(JSONReportBody jsonReport, XMLReportBody xmlReport) {
		if (jsonReport != null) {
			jsonReport = new JSONReportBody();
		}
		if (xmlReport != null) {
			xmlReport = new XMLReportBody();
		}
	...
	public void addContent() {
		...
		if (jsonReport != null) {
			// Add customer info
			jsonReport.addContent("sale:{customer:{");
			jsonReport.addContent("name:\"");
			jsonReport.addContent(saleEntry.getCustomer().getName());
			jsonReport.addContent("\",phone:\"");
			jsonReport.addContent(saleEntry.getCustomer().getPhone());
			jsonReport.addContent("\"}");
			// add array of items
			jsonReport.addContent(",items:[");
			Iterator<SoldItem> it = saleEntry.getSoldItems().iterator();
			while (it.hasNext()) {
				SoldItem item = it.next();
				jsonReport.addContent("{name:\"");
				jsonReport.addContent(item.getName());
				jsonReport.addContent("\",quantity:");
				jsonReport.addContent(String.valueOf(item.getQuantity()));
				jsonReport.addContent(",price:");
				jsonReport.addContent(String.valueOf(item.getUnitPrice()));
				jsonReport.addContent("}");
				if (it.hasNext()) {
					jsonReport.addContent(",");
				}
			...
			jsonReport.addContent("]}");
			report.setReportBody(jsonReport);
		}
		if (xmlReport != null) {
			xmlReport = new XMLReportBody();
		}
